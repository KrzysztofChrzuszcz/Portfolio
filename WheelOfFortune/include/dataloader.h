...
public:
    ...
    virtual void                        setMaxPositions(int maxPositionsAmount) { m_MaxPositionsAmount = maxPositionsAmount; } //!< Sets maximum amount of positions that can be display.
    virtual void                        adjust(const Settings& conf) { std::for_each(m_Entries.begin(), m_Entries.end(), [&](Entry& e) { e.m_Color.adjustBrightness(conf.getMinColorBrightness()); }); } //!< Adjust input data for better visibility. For now I need to adjust only brightness, so it's a good place for simple lambda usage as example.
