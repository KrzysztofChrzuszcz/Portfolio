...
bool TinyDataLoader::loadXml(const char* path)
{
    ...
    XMLDocument doc;
    XMLError err = doc.LoadFile(path); // NOTE: Jest problem z polskimi znakami w sciezce
    ...
    if (XMLElement* rootElem = doc.FirstChildElement("root"))
        if (XMLElement* optionsElem = rootElem->FirstChildElement("options"))
        {
            ...
            do
            {
                const char* title = optionElem->GetText(); // NOTE: Nie radzi sobie z polskimi znakami TODO: Pomyslec nad mozliwym rozwiazaniem
                printf("%s: ", title);
                ...
                catch (...)
                {
                    ...
                    // TODO:  mozna wywolac Warning Window jesli bedzie problem to zwrocic false
    ...
    // TODO1: uzyc w engine do wywolania AlarmLoadingDataProblem w GUI czy co tam bedzie odpowiednie
    // TODO1ad: Okno ostrzegawcze [Data corrupted, for more info check log file]
    // TODO2: dodac logFile'a 
    return fullSuccess; 
}